...
if page == "🏠 Overview":
    ...
    st.caption(f"📅 Periode: {start_date} s/d {end_date}" if start_date else "📅 Semua periode")
    st.markdown("---")
...
# ============================================
    ...
    with col1:
        st.metric("🚀 Lebih Cepat", f"{faster_df['review_score'].mean():.2f}", 
...
# ============================================
    ...
    faster_rating = filtered_delivery[filtered_delivery['delivery_status'] == '🚀 Lebih Cepat']['review_score'].mean()
    late_rating = filtered_delivery[filtered_delivery['delivery_status'] == '⚠️ Terlambat']['review_score'].mean()
    ontime_rating = filtered_delivery[filtered_delivery['delivery_status'] == '✅ Tepat Waktu']['review_score'].mean()
    ...
    correlation = city_performance[['total_sellers', 'total_products_sold']].corr().iloc[0, 1]
    top_city = city_performance.iloc[0]['seller_city']
    top_state = city_performance.iloc[0]['seller_state']
    top_sales = city_performance.iloc[0]['total_products_sold']
